 ...
 */
  ...
  */
class XmlSearchAdapter68Test extends PHPUnit_Framework_TestCase
{
	...
	public function setUp()
	{
		...
		$this->dataProvider->setFileLocation(RESOURCES_DIR.DS.'responses'.DS.'xml68');
		$this->searchAdapter = FF::getInstance('xml68/searchAdapter', $this->dataProvider, self::$paramsParser, self::$encodingHandler);
	}
    ...
    {
        $this->searchAdapter->setParam('query', 'asnStyles');
        
        $asn = $this->searchAdapter->getAsn();
        
        $this->assertTrue($asn[0]->isTreeStyle());
        $this->assertTrue($asn[1]->isSliderStyle());
        $this->assertTrue($asn[2]->isDefaultStyle());
        $this->assertTrue($asn[3]->isMultiSelectStyle());
        $this->assertTrue($asn[4]->isColorStyle());
    }
