<?php
  ...
  */
class JsonSuggestAdapter68Test extends PHPUnit_Framework_TestCase
{
	...
	public function setUp()
	{
		...
		$this->suggestAdapter = FF::getInstance('json68/suggestAdapter', $this->dataProvider, self::$paramsParser, self::$encodingHandler, self::$log);
	}
	...
	public function testSuggestionLoading()
	{
		...
		$this->assertEquals(4, count($suggestions), 'wrong number of suggest queries delivered');
        ...
        $this->assertEquals('bmx sophie', $suggestions[0]->getQuery(), 'wrong query delivered for first suggest item');
        $this->assertEquals('searchTerm', $suggestions[0]->getType(), 'wrong type delivered for first suggest item');
        $this->assertEquals('brand', $suggestions[1]->getType(), 'wrong type delivered for second suggest item');
        $this->assertEquals('category', $suggestions[2]->getType(), 'wrong type delivered for third suggest item');
        $this->assertEquals('productName', $suggestions[3]->getType(), 'wrong type delivered for fourth suggest item');
	}
}